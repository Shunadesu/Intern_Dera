...
const FullPC = () => {
  ...
  return (
    ...
    flex flex-col gap-4 justify-center'>
        ...
        md-max-2:grid-cols-2 py-4
        md-max-3:grid-cols-3
        md-max-4:grid-cols-2 md-max-4:py-4
        justify-items-center
        '>
        {currentProducts.map((product) => (
          <Link to={`/news/${product.id}`} key={product.id} className='w-full flex flex-col gap-3 p-8
          '>
            <div className='w-full h-[300px] 
            md-max-3:w-full md-max-3:h-[220px]
            md-max-4:w-full md-max-4:h-[180px]
            md-max-5:w-full md-max-5:h-[150px]
            '>
